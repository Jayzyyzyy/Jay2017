...
public class Problem_01_TarjanAndDisjointSetsForLCA {
	...
	public static class DisjointSets {
		...
		public void union(Node a, Node b) {
			...
			if (aFather != bFather) {
				int aFrank = rankMap.get(aFather); //集合rank值
				int bFrank = rankMap.get(bFather); //集合rank值
				if (aFrank < bFrank) {
					fatherMap.put(aFather, bFather);  //b大，把a挂在b下
				} else if (aFrank > bFrank) {
					fatherMap.put(bFather, aFather); //a大，把b挂在a下
				} else {
