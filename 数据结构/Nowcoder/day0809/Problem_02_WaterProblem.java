...
public class Problem_02_WaterProblem {
	...
	public static int getWater1(int[] arr) {
		if (arr == null || arr.length < 3) { //开头结尾不能放水
			return 0;
		}
		int value = 0;
	...
	public static int getWater2(int[] arr) {
		...
		int n = arr.length - 2;
		int[] leftMaxs = new int[n]; //左边最大
		leftMaxs[0] = arr[0];
		for (int i = 1; i < n; i++) {
			leftMaxs[i] = Math.max(leftMaxs[i - 1], arr[i]);
		}
		int[] rightMaxs = new int[n]; //右边最大
		rightMaxs[n - 1] = arr[n + 1];
		for (int i = n - 2; i >= 0; i--) {
			rightMaxs[i] = Math.max(rightMaxs[i + 1], arr[i + 2]);
		}
		int value = 0;
		for (int i = 1; i <= n; i++) {//遍历统计
	...
	public static int getWater4(int[] arr) {
		...
		int value = 0;
		int leftMax = arr[0]; //左边最大
		int rightMax = arr[arr.length - 1]; //右边最大
		int l = 1;
		int r = arr.length - 2;
		while (l <= r) {
			if (leftMax <= rightMax) {
